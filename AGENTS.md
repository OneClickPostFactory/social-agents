...
- The target Reddit ingestion path is the normal Reddit authorization connector. The main app owns encrypted per-user Reddit tokens, source records, OpenAI angle extraction, queueing, publishing, logs, and billing. It fetches explicit configured subreddit sources through Reddit's authenticated API and writes normalized source records.
- `fetch_sources` / `refresh_queue` must not treat Reddit public JSON or RSS from the Cloudflare Worker as the normal product path. When no usable source records or banked angles exist, the worker should return `reddit_authorization_required` without calling Reddit public JSON/RSS, OpenAI, queue creation, or publishing.
...
- The old per-source Reddit OAuth credential path is not a product path. Do not add tenant Reddit OAuth credential prompts. App-level Reddit OAuth credentials are deployment configuration for the normal Reddit authorization connector.
